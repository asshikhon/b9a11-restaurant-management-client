...
if(user?.email){
  ...
  const handleDelete = (_id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${import.meta.env.VITE_API_URL}/purchase/${_id}`, {
            headers: {
  ...
  return (
    <div className=" px-3 md:px-0 mb-10 md:mb-16 lg:mb-20">
      ...
      >
        <h2 className="text-4xl text-white font-bold">My Ordered Food Items</h2>
      </div>
            ...
            {item?.map((p, idx) => (
              <tr key={idx}>
                ...
                <td className="text-lg font-semibold">
                  {new Date(p.buyingDate).toLocaleDateString()}
                </td>
